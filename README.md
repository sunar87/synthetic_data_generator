...
Каждое поле имеет тип и параметры.  
Поддерживаемые типы:

| Тип | Назначение | Пример |
|------|-------------|---------|
| `integer` | Целое число | `"id": { "type": "integer" }` |
| `string` | Строка (подтипы: name, word, sentence, paragraph) | `"name": { "type": "string", "params": { "subtype": "name" } }` |
| `boolean` | Булево значение | `"is_vip": { "type": "boolean" }` |
| `reference` | Ссылка на другую сущность | `"user_id": { "type": "reference", "params": { "entity": "users", "field": "id" } }` |
| `one_to_many` | Обратная связь (список связанных объектов) | `"orders": { "type": "one_to_many", "params": { "entity": "orders", "foreign_field": "user_id", "embed": true } }` |
...
- `adjust` — откорректировать значение, если не соответствует минимальному порогу
